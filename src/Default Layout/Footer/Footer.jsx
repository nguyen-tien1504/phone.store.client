import React from "react";
import banner from "../../img/Navbar/navbarbanner.png";
const FooterLinks = [
  {
    links: [
      "Gioi thieu ve cong ty",
      "Cau hoi thuong gap",
      "Chinh sach thuong gap",
      "Quy che hoat dong",
    ],
  },
  {
    links: [
      "Kiem tra hoa don dien tu",
      "Tra cuu thong tin bao hanh",
      "Tin tuyen dung",
      "Tin khuyen mai",
      "Huong dan online",
    ],
  },
  {
    links: [
      "He thong cua hang",
      "He thong bao hanh",
      "Kiem tra hang apple chinh hang",
      "Gioi thieu doi may",
      "Chinh sach doi tra",
    ],
  },
];

const FooterComponents = (props) => {
  return (
    <div className="flex flex-col gap-1">
      {props.link.map((item,index) => {
        return (
          <a href="" className="text-[#f2f2f2] font-thin text-lg" key={index}>
            {item}
          </a>
        );
      })}
    </div>
  );
};
const Footer = () => {
  return (
    <div className="mt-5 py-5 flex gap-8 bg-[#333333]">
      <div>
        <img src={banner} alt="" className="w-fit h-fit" />
      </div>
      {FooterLinks.map((item,index) => {
        return <FooterComponents link={item.links} key={index}/>;
      })}

      <div className="">
        <p className="uppercase text-white">social media</p>
        <div className="flex gap-2 justify-between mt-4">
          <a href="">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="60"
              height="60"
              fill="#fff"
              className="bi bi-facebook"
              viewBox="0 0 16 16"
            >
              <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z" />
            </svg>
          </a>

          <a href="">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="60"
              height="60"
              fill="#fff"
              className="bi bi-google"
              viewBox="0 0 16 16"
            >
              <path d="M15.545 6.558a9.42 9.42 0 0 1 .139 1.626c0 2.434-.87 4.492-2.384 5.885h.002C11.978 15.292 10.158 16 8 16A8 8 0 1 1 8 0a7.689 7.689 0 0 1 5.352 2.082l-2.284 2.284A4.347 4.347 0 0 0 8 3.166c-2.087 0-3.86 1.408-4.492 3.304a4.792 4.792 0 0 0 0 3.063h.003c.635 1.893 2.405 3.301 4.492 3.301 1.078 0 2.004-.276 2.722-.764h-.003a3.702 3.702 0 0 0 1.599-2.431H8v-3.08h7.545z" />
            </svg>
          </a>
        </div>
      </div>

      <div className="flex items-center self-end gap-2 ">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="20"
          height="20"
          fill="#fff"
          className="bi bi-c-circle-fill"
          viewBox="0 0 16 16"
        >
          <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0ZM8.146 4.992c.961 0 1.641.633 1.729 1.512h1.295v-.088c-.094-1.518-1.348-2.572-3.03-2.572-2.068 0-3.269 1.377-3.269 3.638v1.073c0 2.267 1.178 3.603 3.27 3.603 1.675 0 2.93-1.02 3.029-2.467v-.093H9.875c-.088.832-.75 1.418-1.729 1.418-1.224 0-1.927-.891-1.927-2.461v-1.06c0-1.583.715-2.503 1.927-2.503Z" />
        </svg>
        <p className="text-white">HAT Shop</p>
      </div>
    </div>
  );
};

export default Footer;
