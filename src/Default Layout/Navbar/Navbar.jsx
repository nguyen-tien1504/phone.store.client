...
const NavbarLink = (props) => {
  return (
    <li className="uppercase">
      <Link to={{ pathname: "/products" }} state={{ name: props.name }}>
        {props.name}
      </Link>
    </li>
  );
    ...
    <div>
      <div className="bg-[#333333] flex items-center place-content-between">
        <div className="flex items-center">
          <Link to="/">
            <img src={banner} alt="Navbar banner" className="w-fit h-fit" />
          </Link>
        ...
        <div className="flex items-center relative gap-x-9 mr-10">
          ...
          {show && user ? (
            <div className="bg-slate-200 flex flex-col gap-1 items-start p-2 text-base rounded absolute top-full left-0">
              <Link to="/user-detail" onClick={() => setShow(!show)}>
                Thông tin khách hàng
              </Link>
              <button onClick={handleUser}>Đăng xuất</button>
            </div>
          ) : null}
          <Link to="/cart-shopping" className="text-white realative">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
      ...
      <div className="bg-[orange]/70">
        <ul className="flex gap-x-4 items-center justify-center">
          <NavbarLink name="Tất cả" />
          <NavbarLink name="iphone" />
          <NavbarLink name="samsung" />
          <NavbarLink name="oppo" />
          <NavbarLink name="huawei" />
          <NavbarLink name="realme" />
          <NavbarLink name="vivo" />
          <NavbarLink name="xiaomi" />
          <NavbarLink name="nokia" />
        </ul>
      </div>
    </div>
  );
